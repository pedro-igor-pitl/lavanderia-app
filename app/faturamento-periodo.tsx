...
export default function FaturamentoPeriodo() {
    ...
    const totalReaisPeso = rollsPeso.reduce(
        (acc, item) =>
            acc + (Number(item.peso || 0) * Number(item.precoUnitario || 0)),
        0
    );

    const totalReaisPeca = rollsPeca.reduce(
        (acc, item) =>
            acc + (Number(item.quantidade || 0) * Number(item.precoUnitario || 0)),
        0
    );
        ...
        {loading ? (
          ...
          <>
            ...
            {rollsPeso.map((item, index) => (
                ...
                <Text style={styles.cell}>
                    R$ {(Number(item.peso || 0) * Number(item.precoUnitario || 0)).toFixed(2)}
                </Text>
            ...
            <View style={styles.tableContainer}>
                ...
                {rollsPeca.map((item, index) => (
                    ...
                    <Text style={styles.cell}>
                        R$ {(Number(item.quantidade || 0) * Number(item.precoUnitario || 0)).toFixed(2)}
                    </Text>
